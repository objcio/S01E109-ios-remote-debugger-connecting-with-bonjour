...
extension SplitViewController {
	...
	func render(callback: @escaping (Message) -> (), viewController vc: UISplitViewController) -> StrongReferences {
		...
		let detail: ViewController<Message> = .navigationController(right(vc.viewControllers.count == 1 ? nil : vc.displayModeButtonItem), modal: nil)
		strongReferences.append(detail.render(callback: callback, change: &delegate.detailViewController))
		assert(vc.viewControllers.count == 1 || vc.viewControllers[1] == delegate.detailViewController)
...
extension ViewController {
	func render(callback: @escaping (Message) -> (), change: inout UIViewController) -> StrongReferences {
		switch self {
		case let .navigationController(newNC, modal):
			var n: UINavigationController! = change as? UINavigationController
			if n == nil {
				n = UINavigationController()
				change = n
			}
			var strongReferences = newNC.render(callback: callback, viewController: n)
			if let modal = modal {
				if var v = n.presentedViewController {
					strongReferences.append(modal.viewController.render(callback: callback, change: &v))
					assert(v == n.presentedViewController)
				} else {
					...
					n.present(vc, animated: true, completion: nil)
				}
			} else {
				if let vc = n.presentedViewController, !(vc is UIAlertController) {
...
extension NavigationController {
	func render(callback: @escaping (Message) -> (), viewController nc: UINavigationController) -> StrongReferences {
		...
		if diffN < 0 { // pop unnessary vc's
			nc.viewControllers = Array(nc.viewControllers.prefix(viewControllers.count))
		}
