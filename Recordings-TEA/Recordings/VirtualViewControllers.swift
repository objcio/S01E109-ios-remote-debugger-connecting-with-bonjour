...
indirect enum ViewController<Message> {
	...
	case navigationController(NavigationController<Message>, modal: Modal<Message>?)
	...
	func map<B>(_ transform: @escaping (Message) -> B) -> ViewController<B> {
		switch self {
		case .navigationController(let nc, let modal): return .navigationController(nc.map(transform), modal: modal?.map(transform))
