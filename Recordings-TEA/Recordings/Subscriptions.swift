...
enum Subscription<A> {
	case playProgress(handle: (TimeInterval?, _ isPlaying: Bool) -> A)
...
extension Subscription {
	func map<B>(_ f: @escaping (A) -> B) -> Subscription<B> {
		switch self {
		case let .playProgress(handle: handle):
			return .playProgress(handle: { f(handle($0, $1)) })
...
final class SubscriptionManager<Message> {
	...
	func update(subscriptions: [Subscription<Message>], player: Player?) {
		// Todo: here we should reuse existing observers, if possible?
		var newStoreObservers: [Any] = []
		for subscription in subscriptions {
			switch subscription {
			case .playProgress(let f):
				player?.update = { [weak self] position, isPlaying in self?.callback(f(position, isPlaying)) }
